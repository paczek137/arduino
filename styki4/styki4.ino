...
int main(void) {
  ...
  while(1) {
    ...
    if(!key1_lock && (BUTTON_1_PINx & (1<<BUTTON_1_PIN))) {
      key1_lock = 1;
      
      LED_1_PORT ^= (1<<LED_1_PIN);
    } else if(key1_lock && !(BUTTON_1_PINx & (1<<BUTTON_1_PIN))) {
      key1_lock++;
    }
    
    if(!key2_lock && (BUTTON_2_PINx & (1<<BUTTON_2_PIN))) {
      key2_lock = 1;
      
    } else if(key2_lock && !(BUTTON_2_PINx & (1<<BUTTON_2_PIN))) {
      if(!++key2_lock) {
